...
const ImageUpload: React.FC = () => {
  ...
  const handleUploadSuccess = (result: any, { widget }: any) => {
    ...
    console.log('Uploaded Resource:', result?.info);  // Log the uploaded resource
     axios.post('/api/upload',{ email: localStorage.getItem('email'), profilePicture: result?.info.secure_url }).then((res) => console.log(res.data)).catch((err) => console.error(err));
  };
